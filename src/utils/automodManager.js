...
class AutomodManager {
    ...
    async syncNativeRules(guildId, settings) {
        try {
            ...
            if (!rules) return;
            const nsfwRuleName = "Voltix Safety: NSFW Filter";
            ...
            if (settings.antiNsfw && !existingNsfw) {
                await guild.autoModerationRules.create({
                    ...
                    actions: [{
                        type: 1,
                        metadata: { customMessage: "This message was blocked by Voltix's NSFW Filter." }
                    }],
                    enabled: true,
    ...
    async takeAction(message, reason, settings, action) {
        ...
        let dmPayload = null;
        const welcomerData = this.client.db.welcomemessages.get(message.guild.id);
        const customWarnMsg = welcomerData ? welcomerData.warnMessage : null;

        if (customWarnMsg && customWarnMsg.trim() && escType === 'warn') {
            ...
            dmPayload = parseMessage(customWarnMsg.trim(), replacements);
        } else {
        ...
        const dmed = await message.author.send(dmPayload).then(() => true).catch(() => false);
