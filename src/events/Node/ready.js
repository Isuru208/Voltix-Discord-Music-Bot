  ...
  run: async (client, name) => {
    ...
    for (const data of maindata) {
      try {
        ...
        // Avoid stomping on existing active players and their queues
        if (client.manager.players.has(data.guildId)) {
          client.logger.log(
            `Auto Reconnect: Player already active for guild ${data.guildId}. Skipping reconnection.`,
            "log"
          );
          continue;
        }
