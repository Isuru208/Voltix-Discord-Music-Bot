  ...
  run: async (client, player, track) => {
    try {
      ...
      if (autoplay && track) {
        ...
        let nextTrack = null;
        let ytRecommendations = [];

        // 1. Try to fetch recommendations from YouTube if the track is a YouTube track
        const isYoutubeId = track.identifier && /^[a-zA-Z0-9_-]{11}$/.test(track.identifier);
        if (isYoutubeId) {
          try {
            const axios = require("axios");
            const res = await axios.get(`https://www.youtube.com/watch?v=${track.identifier}`, {
              headers: {
                ...
                if (typeof obj === 'object') {
                  if (obj.videoId && obj.title) {
                    ...
                    if (typeof obj.title === 'object') {
                      if (obj.title.runs && obj.title.runs[0]) titleText = obj.title.runs[0].text;
                      else if (obj.title.simpleText) titleText = obj.title.simpleText;
                    } else if (typeof obj.title === 'string') {
                      titleText = obj.title;
                    }
                    if (titleText && !ytRecommendations.some(v => v.videoId === obj.videoId)) {
                      ytRecommendations.push({ videoId: obj.videoId, title: titleText });
                    }
                  }
                  for (const k in obj) {
                    search(obj[k]);
            ...
            console.error("[Autoplay] YouTube recommendations fetch error:", err.message);
          }
        }

        const findDirectTrack = async (videoId) => {
          try {
            const query = `https://www.youtube.com/watch?v=${videoId}`;
            const result = await client.manager.search(query, { requester: client.user });
        ...
        if (ytRecommendations.length > 0) {
          // Resolve first few videos in parallel to find a valid track quickly
          const searchPromises = ytRecommendations.slice(0, 3).map(r => findDirectTrack(r.videoId));
          const results = await Promise.all(searchPromises);
          nextTrack = results.find(t => t !== null);

          // Fallback to sequential resolve for other recommended videos
          if (!nextTrack && ytRecommendations.length > 3) {
            for (let i = 3; i < ytRecommendations.length; i++) {
              nextTrack = await findDirectTrack(ytRecommendations[i].videoId);
              if (nextTrack) break;
            }
          }
        }

        // 2. Fall back to Last.fm recommendations if YouTube extraction yielded nothing
        if (!nextTrack) {
        ...
        if (nextTrack) {
          ...
          console.log(`[Autoplay] No tracks found for recommendations.`);
        }
      }
    } catch (error) {
