...
module.exports = {
  ...
  run: async (client, message) => {
    ...
    if (!command) {
      const tag = client.db.tags.get(message.guild.id, commandName);
      if (tag && tag.content) {
        const replacements = {
        ...
        const messagePayload = parseMessage(tag.content, replacements);
        return message.channel.send(messagePayload).catch(() => null);
      }
      return;
    }
