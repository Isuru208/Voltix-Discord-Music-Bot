...
module.exports = {
  ...
  run: async (client) => {
    ...
    // Dashboard presence update integration
    const fs = require('fs');
    const path = require('path');
    const settingsPath = path.join(__dirname, '../../dashboard/settings.json');

    const updatePresence = () => {
      try {
        let customPresence = null;
        if (fs.existsSync(settingsPath)) {
          customPresence = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
        }

        if (customPresence && customPresence.enabled) {
          const typeMap = {
          ...
          client.user.setPresence({
            activities: [
              {
                name: customPresence.activityName || "Voltix Music",
                type: typeMap[customPresence.activityType] !== undefined ? typeMap[customPresence.activityType] : ActivityType.Custom,
                url: customPresence.activityType === 1 ? (customPresence.streamUrl || undefined) : undefined
              },
            ],
            status: customPresence.status || "online",
          });
        } else {
        ...
        console.error("Error updating presence in ready event:", err);
      }
    };

    // Run once on startup
    updatePresence();
    // And run periodically every 15 seconds
    setInterval(updatePresence, 15000);

    // Initialize Dashboard Server on primary cluster (Cluster 0)
    if (client.cluster.id === 0) {
      try {
        const startDashboard = require("../../dashboard/server.js");
        startDashboard(client);
      } catch (err) {
        client.logger.log(`Failed to start Web Dashboard: ${err.message}`, "error");
      }
    }
  },
};
