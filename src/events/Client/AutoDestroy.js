...
module.exports = {
  ...
  run: async (client, oldState, newState) => {
    ...
    // Gaming Mode: Auto-duck volume when members talk/unmute
    if (player && player.data.get("gamingMode")) {
      try {
        const guild = client.guilds.cache.get(player.guildId);
        if (guild) {
          const voiceChannel = guild.channels.cache.get(player.voiceId);
          if (voiceChannel) {
            // Find all members in the voice channel except the bot itself
            const members = voiceChannel.members.filter(m => m.id !== client.user.id);
            // Check if any member is NOT selfMute and NOT serverMute
            const hasActiveTalkers = members.some(m => !m.voice.selfMute && !m.voice.serverMute);
            const isDucked = player.data.get("isDucked") || false;

            if (hasActiveTalkers && !isDucked) {
              // Duck the player volume to 20%
              player.data.set("preDuckVolume", player.volume);
              player.setVolume(20);
              player.data.set("isDucked", true);
            } else if (!hasActiveTalkers && isDucked) {
        ...
        console.error(`Error handling gaming mode in voiceStateUpdate: ${err}`);
      }
    }
  },
};
