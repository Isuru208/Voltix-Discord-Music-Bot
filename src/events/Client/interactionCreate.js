...
const MUSIC_BUTTON_IDS = new Set([
  "pause", "resume", "skip", "stop", "previous", "like",
  "vol_down", "vol_up", "mute_toggle", "game_mode_toggle"
]);
...
module.exports = {
  ...
  run: async (client, interaction) => {
    ...
    if (interaction.isButton()) {
      // Global music button handler — works even after bot restart (no collector needed)
      if (MUSIC_BUTTON_IDS.has(interaction.customId)) {
        try {
          const player = client.manager?.players?.get(interaction.guildId);
          if (!player || !player.queue?.current) {
            const display = new TextDisplayBuilder()
              .setContent(`**${client.emoji.warn} No music is currently playing.**`);
          ...
          const { handleButtonInteractionGlobal } = require("../Players/playerStart");
          await handleButtonInteractionGlobal(interaction, player, client);
        } catch (err) {
          console.error('[GlobalMusicButton] Error:', err);
          if (!interaction.replied && !interaction.deferred) {
            const display = new TextDisplayBuilder()
