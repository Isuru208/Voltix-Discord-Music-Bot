...
const tables = [
    ...
    },
    {
        ...
        schema: `
            guildId TEXT PRIMARY KEY,
            welcomeChannel TEXT,
            welcomeMessage TEXT DEFAULT 'Welcome {user} to {guild}!',
            goodbyeChannel TEXT,
            goodbyeMessage TEXT DEFAULT '{username} has left the server.',
            enabled INTEGER DEFAULT 0,
            dmOnJoinMessage TEXT,
            banMessage TEXT,
            warnMessage TEXT,
            dmOnLeaveMessage TEXT
        `
    },
    {
        name: 'tags',
    ...
    },
    {
        ...
        schema: `
            guildId TEXT PRIMARY KEY,
            messageDeleteChannel TEXT,
            messageEditChannel TEXT,
            memberJoinLeaveChannel TEXT,
            roleUpdateChannel TEXT,
            enabled INTEGER DEFAULT 0
        `
    },
    {
...
tables.forEach(table => {
    ...
    if (table.name === 'welcomemessages') {
        const columns = db.prepare('PRAGMA table_info(welcomemessages)').all();
        const columnNames = columns.map(c => c.name);
        const required = ['dmOnJoinMessage', 'banMessage', 'warnMessage', 'dmOnLeaveMessage'];
        required.forEach(col => {
            if (!columnNames.includes(col)) {
                db.prepare(`ALTER TABLE welcomemessages ADD COLUMN ${col} TEXT`).run();
            }
        });
    }
});
...
const indexes = [
    ...
    'CREATE INDEX IF NOT EXISTS idx_welcomemessages_guildId ON welcomemessages(guildId)',
    'CREATE INDEX IF NOT EXISTS idx_tags_guildId ON tags(guildId)',
    'CREATE INDEX IF NOT EXISTS idx_logging_guildId ON logging(guildId)',
    'CREATE INDEX IF NOT EXISTS idx_reactionroles_guildId ON reactionroles(guildId)'
];
...
managers.welcomemessages = {
    get: (guildId) => {
        const row = db.prepare('SELECT * FROM welcomemessages WHERE guildId = ?').get(guildId);
        if (!row) return null;
        return {
            ...row,
            enabled: !!row.enabled
    ...
    set: (guildId, data) => {
        ...
        const exists = db.prepare('SELECT 1 FROM welcomemessages WHERE guildId = ?').get(guildId);
        if (exists) {
            params.push(guildId);
            db.prepare(`UPDATE welcomemessages SET ${updates.join(', ')} WHERE guildId = ?`).run(...params);
        } else {
            const keys = ['guildId', ...Object.keys(data).filter(k => k !== 'guildId')];
            const vals = keys.map(k => {
                let v = k === 'guildId' ? guildId : data[k];
            ...
            db.prepare(`INSERT INTO welcomemessages (${keys.join(', ')}) VALUES (${keys.map(() => '?').join(', ')})`).run(...vals);
        }
    }
};
...
managers.tags = {
    ...
    getForGuild: (guildId) => {
        return db.prepare('SELECT * FROM tags WHERE guildId = ?').all(guildId);
    },
    set: (guildId, name, content) => {
        db.prepare('INSERT OR REPLACE INTO tags (guildId, name, content) VALUES (?, ?, ?)').run(guildId, name, content);
    },
    delete: (guildId, name) => {
        db.prepare('DELETE FROM tags WHERE guildId = ? AND name = ?').run(guildId, name);
    },
    deleteForGuild: (guildId) => {
        db.prepare('DELETE FROM tags WHERE guildId = ?').run(guildId);
    }
};

managers.logging = {
    get: (guildId) => {
        const row = db.prepare('SELECT * FROM logging WHERE guildId = ?').get(guildId);
        if (!row) return null;
        return {
            ...row,
            enabled: !!row.enabled
    ...
    set: (guildId, data) => {
        ...
        const exists = db.prepare('SELECT 1 FROM logging WHERE guildId = ?').get(guildId);
        if (exists) {
            params.push(guildId);
            db.prepare(`UPDATE logging SET ${updates.join(', ')} WHERE guildId = ?`).run(...params);
        } else {
            const keys = ['guildId', ...Object.keys(data).filter(k => k !== 'guildId')];
            const vals = keys.map(k => {
                let v = k === 'guildId' ? guildId : data[k];
            ...
            db.prepare(`INSERT INTO logging (${keys.join(', ')}) VALUES (${keys.map(() => '?').join(', ')})`).run(...vals);
        }
    }
};

managers.reactionroles = {
    get: (guildId, messageId, emoji) => {
        return db.prepare('SELECT * FROM reactionroles WHERE guildId = ? AND messageId = ? AND emoji = ?').get(guildId, messageId, emoji);
    },
    getForGuild: (guildId) => {
        return db.prepare('SELECT * FROM reactionroles WHERE guildId = ?').all(guildId);
    },
    set: (guildId, messageId, channelId, emoji, roleId) => {
        db.prepare('INSERT OR REPLACE INTO reactionroles (guildId, messageId, channelId, emoji, roleId) VALUES (?, ?, ?, ?, ?)').run(guildId, messageId, channelId, emoji, roleId);
    },
    delete: (guildId, messageId, emoji) => {
        db.prepare('DELETE FROM reactionroles WHERE guildId = ? AND messageId = ? AND emoji = ?').run(guildId, messageId, emoji);
    },
    deleteForGuild: (guildId) => {
        db.prepare('DELETE FROM reactionroles WHERE guildId = ?').run(guildId);
    }
};
