...
async function createBackup() {
    ...
    const backupPath = path.join(rootDir, `voltix_backup.zip`);

    fs.readdirSync(rootDir).forEach(file => {
        if ((file.startsWith('backup_') && file.endsWith('.zip')) || file === 'voltix_backup.zip') {
    ...
    return new Promise((resolve, reject) => {
        ...
        const filesToBackup = [
            ...
            { path: path.join(rootDir, 'database.db'), name: 'database.db' },
            { path: path.join(rootDir, 'index.js'), name: 'index.js' },
            { path: path.join(rootDir, 'config.eps'), name: 'config.eps' },
            { path: path.join(rootDir, 'package.json'), name: 'package.json' },
            { path: path.join(rootDir, 'database.db-shm'), name: 'database.db-shm' },
            { path: path.join(rootDir, 'database.db-wal'), name: 'database.db-wal' }
        ];
