    ...
    try {
      ...
      if (position === 0) {
        try {
          ...
          console.error('[Play Command] Failed to delete deferred reply:', err);
        }
        return;
      }
      ...
      const infoDisplay = new TextDisplayBuilder()
        .setContent(
          ...
          `-# Position \` #${position === 1 && isPlaying ? "1 (Next)" : position} \` • Duration \` ${convertTime(track.length)} \` • By \` ${interaction.user.username} \``
        );
    ...
    try {
      ...
      if (!player) {
        try {
          ...
          console.error("Player creation error:", createError);
          console.log(`Attempting automated fix for guild ${message.guild.id}...`);

          try {
            const { recreatePlayer } = require("../../utils/playerUtils");
            player = await recreatePlayer(client, message.guild.id, channel.id, message.channel.id);
            console.log(`Successfully fixed player for guild ${message.guild.id}`);
          } catch (retryError) {
            console.error("Automated fix failed:", retryError);
            throw new Error(`Voice connection failed: ${createError.message}. Automated fix also failed: ${retryError.message}`);
          }
      ...
      if (searchResult.type === "PLAYLIST") {
        ...
        addedTracks.push({ track: track, position });
        trackCounter++;
      }
      ...
      if (searchResult.type === "PLAYLIST") {
        ...
        const infoDisplay = new TextDisplayBuilder()
          .setContent(
            ...
            `> Position \` #${track.position === 1 && isPlaying ? "1 (Next)" : track.position} \` • Duration \` ${convertTime(track.track.length)} \` • By \` ${message.author.username} \``
          );
...
async function performSmartSelection(query, requester, client) {
  ...
  const searchPromises = shuffledSources.map(async (source) => {
    try {
      ...
      const resolvePromise = node.rest.resolve(searchQuery);
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Resolve timeout')), 1500)
      );
      
      const res = await Promise.race([resolvePromise, timeoutPromise]);
