...
module.exports = {
  ...
  slashOptions: [
    {
      name: "amount",
      description: "Volume amount (0-150)",
      type: 4,
      required: false
    }
  ],
  ...
  async slashExecute(interaction, client) {
    ...
    if (volume < 0 || volume > 150) {
      const errorDisplay = new TextDisplayBuilder().setContent(`**${client.emoji.cross} Volume must be between 0 and 150.**`);
      const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
      return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
    }
  ...
  async execute(message, args, client, prefix) {
    ...
    if (args.length) {
      ...
      if (isNaN(volume) || volume < 0 || volume > 150) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(
            `**${client.emoji.cross} Usage** \`:\` \`${prefix}volume [0-150]\`\n` +
    ...
    collector.on("collect", async (interaction) => {
      try {
        ...
        switch (interaction.customId) {
          case "vol_plus":
            newVolume = Math.min(150, currentPlayer.volume + 10);
            break;
          case "vol_minus":
