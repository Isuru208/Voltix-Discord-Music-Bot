    ...
    async slashExecute(interaction, client) {
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === confirmId) {
                ...
                let dmSent = false;
                try {
                    const dbData = client.db.welcomemessages.get(interaction.guild.id);
                    const customBanMsg = dbData ? dbData.banMessage : null;

                    if (customBanMsg && customBanMsg.trim()) {
                        ...
                        const dmPayload = parseMessage(customBanMsg.trim(), replacements);
                        await targetUser.send(dmPayload).catch(() => null);
                    } else {
    ...
    async execute(message, args, client) {
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === confirmId) {
                ...
                let dmSent = false;
                try {
                    const dbData = client.db.welcomemessages.get(message.guild.id);
                    const customBanMsg = dbData ? dbData.banMessage : null;
